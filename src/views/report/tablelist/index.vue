    ...
    >
      ...
      <el-table-column label="文章标题">
        <template slot-scope="scope">
          ...
          <router-link :to="{name:'Reporttableshow',params:{id:scope.row.id}}" style="color:darkblue;font-size: 16px;">{{ scope.row.name }}</router-link>
        </template>
      </el-table-column>
  ...
  methods: {
    fetchData() {
      ...
      getList().then(response => {
        ...
        this.$axios.get('/api/report/getpagelist',{  //params参数必写 , 如果没有参数传{}也可以
                        params: {  
    ...
    //获取数据表中所有数据
    getAllList(){
      this.$axios.get('/api/report/gettable').then(res=>{  //这里使用箭头函数的形式
    ...
    //获取条目总数
    getTotal(){
      this.$axios.get('/api/report/gettotal').then(res=>{  //这里使用箭头函数的形式
